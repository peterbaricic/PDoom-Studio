...
// manifest's `thumbs` lists those that show the chapter's current code (a strip of older or newer code, say after a
// restore, isn't listed), with when each was written: only those are asked for (no 404s), and the URL names both the
// chapter's revision and that time, so new code or a refreshed strip is never an old cached picture.
...
export function chapterThumbs(manifest: Manifest | undefined): Record<number, string> {
  ...
  for (const [n, { mtime, revision }] of Object.entries(manifest?.thumbs ?? {})) {
    ...
    out[+n] = `/thumbs/${encodeURIComponent(manifest!.id)}/c0${n}.jpg?r=${revision}.${mtime}`;
  }
  return out;
}
