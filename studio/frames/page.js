...
// uncaught errors. waitUntil is goto's (render.mjs waits for the network to go idle; the pool, which paints straight
// away, doesn't wait for the song to finish buffering). recordScriptErrors keeps { file, message } for each uncaught
// error in window.scriptErrors, so the pool can tell which chapter's script threw while the page loaded.
// Resolves once window.ready is true; the caller checks window.loadError.
export async function openSealedPage(browser, url, { onConsole, onPageError, waitUntil = 'networkidle0', readyTimeout = 60000, recordScriptErrors = false } = {}) {
  ...
  try {
    ...
    await page.waitForFunction('window.ready === true', { timeout: readyTimeout, polling: 100 });
  } catch (e) {
