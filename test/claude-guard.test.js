...
const fake = `bun ${join(process.cwd(), 'test/fake-claude.js')}`;
...
test('an app made without a claudeBin asks the fake whether Claude is signed in', async () => {
  ...
  expect(process.env.CLAUDE_BIN).toBe(fake);
  const app = createApp({ db: openDb(':memory:'), root: process.cwd(), data: tempDir(), token: 'tok', queue: {}, events: createEvents(), port: 18999 });
